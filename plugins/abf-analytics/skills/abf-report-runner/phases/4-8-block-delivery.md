...
4. **Visual** (when visuals are on).
   ...
   c. Otherwise produce the block's default visual per `../../abf-gateway/knowledge/reference/visuals-playbook.md`. Any genuine reporting gap in a time series renders as a true break (`connectNulls: false` / `spanGaps: false`), never interpolated. Line charts render as **no visible point markers** (`pointRadius: 0`).
